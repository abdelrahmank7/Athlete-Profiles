...
export function addFormAndTableEventListeners(athleteId) {
  ...
  setInterval(() => {
    console.log("Interval - Athlete ID:", window.currentAthleteID);
  ...
  if (table) {
    table.addEventListener("focusout", (event) => {
      ...
      if (cell && cell.tagName.toLowerCase() === "td" && cell.cellIndex === 3) {
...
export function addNotesEventListeners() {
  ...
  editButtons.forEach((button) => {
    button.addEventListener("click", (event) => {
      const athleteId = event.target.dataset.athleteId;
      const noteId = event.target.dataset.noteId;
      showEditModal(athleteId, noteId);
    });
  });
}
...
export function initializeEventListeners(athleteId) {
  addFormAndTableEventListeners(athleteId);
